...
defmodule Gossip_Protocol do
  ...
  module. It also includes the node and failure analysis.
  ...
  #def main(args \\ []) do
    ...
    if totNodes > 1 do
        ...
        fail_percent = trunc(arg4 * totNodes * 0.01) # Percentage of failure nodes 

        case algo do
          "gossip" ->
                IO.puts "Running Gossip algorithm"
                actors = init_actors(totNodes)
                init_algorithm(actors, topology, totNodes, algo, fail_percent)
  ...
  def init_algorithm(actors, topology, totNodes, algo, fail_percent) do
    ...
    if (algo == "gossip") do
      ...
      remove_some_actors(actors, fail_percent)
      spawn(__MODULE__, :set_timer, [])
      gossip(actors, neighbors, totNodes, fail_percent)
    else
      # call push-sum algorithm
      remove_some_actors(actors, fail_percent)
      spawn(__MODULE__, :set_timer, [])
      push_sum(actors, neighbors, totNodes, fail_percent)
    end
  ...
  #  -------------------------------   Start Gossip   -------------------------------  
  def gossip(actors, neighbors, totNodes, fail_percent) do
    ...
    if(timeout) do
      #[{_, spread}] = :ets.lookup(:count, "spread")
      IO.inspect "Spread is " <> to_string(spread * 100/totNodes) <> " %"
    else
  ...
  #  ---------------------   Start Push Sum   ---------------------
  def push_sum(actors, neighbors, totNodes, fail_percent) do
    #for  {magicN, y}  <-  neighbors  do
  ...
  #  ---------------   Remove some actors for failure analysis  ---------------
  def remove_some_actors(actors, numofFailNodes) do
    Enum.each(Enum.take_random(actors, numofFailNodes), fn x -> [{_ , failed}] = :ets.lookup(:count, "failed")
